    ...
    match &kind {
      ...
      TokenKind::Bold(n) => {
        ...
        let inner = collect_inline(p, &|k| {
          is_top_level_break(k) || matches!(k, TokenKind::Bold(m) if *m == open_n)
        });
        if matches!(p.peek_kind(), Some(TokenKind::Bold(m)) if *m == open_n) {
          p.advance();
        }
        out.push(Node::Bold(Inline { children: inner, span: default_span() }));
      },
      TokenKind::Italic(n) => {
        let open_n = *n;
        p.advance(); // opener
        let inner = collect_inline(p, &|k| {
          is_top_level_break(k) || matches!(k, TokenKind::Italic(m) if *m == open_n)
        });
        if matches!(p.peek_kind(), Some(TokenKind::Italic(m)) if *m == open_n) {
          p.advance();
        }
        out.push(Node::Italic(Inline { children: inner, span: default_span() }));
      },
      TokenKind::Strike(n) => {
        let open_n = *n;
        p.advance(); // opener
        let inner = collect_inline(p, &|k| {
          is_top_level_break(k) || matches!(k, TokenKind::Strike(m) if *m == open_n)
        });
        if matches!(p.peek_kind(), Some(TokenKind::Strike(m)) if *m == open_n) {
          p.advance();
        }
        out.push(Node::Strikethrough(Inline { children: inner, span: default_span() }));
      },
      TokenKind::CodeStart(n) => {
        ...
        out.push(Node::InlineCode(InlineCode { value, span: default_span() }));
      },
      TokenKind::Bracket => {
        let start = p.pos;
        p.advance(); // [
        ...
        if !matches!(p.peek_kind(), Some(TokenKind::Bracket)) {
          ...
          p.pos = start;
          out.push(Node::Text(Text { value: "[".into(), span: default_span() }));
          p.advance();
          continue;
        }
        p.advance(); // ]
      ...
      TokenKind::Bang => {
        ...
        out.push(Node::Image(Image { src, alt, title: None, span: default_span() }));
      },
