...
pub(crate) fn parse_jsx(p: &mut Parser) -> Node {
  ...
  match p.peek_kind() {
    Some(TokenKind::JsxSelfClosingEnd) => {
      p.advance();
      return Node::JsxSelfClosing(JsxSelfClosing { name, attrs, span: default_span() });
    },
  ...
  if name.is_empty() {
    Node::JsxFragment(JsxFragment { children, span: default_span() })
  } else {
    Node::JsxElement(JsxElement { name, attrs, children, span: default_span() })
  }
}
...
fn parse_attrs(p: &mut Parser) -> Vec<JsxAttr> {
  ...
  while let Some(TokenKind::JsxAttributeName) = p.peek_kind() {
    ...
    out.push(JsxAttr { name, value, span: default_span() });
  }
  out
}
...
pub(crate) fn parse_jsx_expression(p: &mut Parser) -> Node {
  ...
  Node::JsxExpression(JsxExpression { value: s, span: default_span() })
}
