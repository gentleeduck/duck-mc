...
//!
//! These assertions check the *shape* of `dmc::compile` output — not byte
...
fn try_compile(src: &str) -> Option<dmc::CompileOutput> {
  std::panic::catch_unwind(|| dmc::compile(src)).ok()
}
