...
fn frontmatter_extracted() {
  let src = "---\ntitle: T\n---\n# H";
  let out = compile(src);
  assert_eq!(out.frontmatter.get("title").and_then(|v| v.as_str()), Some("T"));
...
fn excerpt_truncates_long_text() {
  ...
  assert!(out.excerpt.chars().count() <= 261, "got {} chars", out.excerpt.chars().count());
}

#[test]
fn body_is_js_function() {
  let out = dmc::compile("# H");
  assert!(out.body.contains("_createMdxContent"));
  assert!(out.body.contains("jsxs(\"h1\""));
}

// frontmatter validation test removed — dmc-schema rebuilt in U2 with new API
