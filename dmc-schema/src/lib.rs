...
pub mod s {
  ...
  pub fn enum_(variants: Vec<Value>) -> EnumSchema {
    EnumSchema { variants }
  }
  pub fn literal(expected: Value) -> LiteralSchema {
    LiteralSchema { expected }
  }
  pub fn union(variants: Vec<Box<dyn Schema>>) -> UnionSchema {
    UnionSchema { variants }
  }
  pub fn record(value: Box<dyn Schema>) -> RecordSchema {
    RecordSchema { value }
  }
  pub fn tuple(items: Vec<Box<dyn Schema>>) -> TupleSchema {
    TupleSchema { items }
  }
  ...
  pub fn coerce_string() -> CoerceSchema {
    CoerceSchema { target: CoerceTarget::String }
  }
  pub fn coerce_number() -> CoerceSchema {
    CoerceSchema { target: CoerceTarget::Number }
  }
  pub fn coerce_boolean() -> CoerceSchema {
    CoerceSchema { target: CoerceTarget::Boolean }
  }
  pub fn coerce_date() -> CoerceSchema {
    CoerceSchema { target: CoerceTarget::Date }
  }

  pub fn optional(inner: Box<dyn Schema>) -> OptionalSchema {
    OptionalSchema { inner }
  }
  pub fn nullable(inner: Box<dyn Schema>) -> NullableSchema {
  ...
  where
    F: Fn(&Value, &mut Vec<String>) + Send + Sync + 'static,
  {
    SuperRefineSchema { inner, predicate: Box::new(predicate) }
  }

  pub fn raw() -> RawSchema {
    RawSchema
  }
  pub fn markdown() -> MarkdownSchema {
    MarkdownSchema
  }
  pub fn mdx() -> MdxSchema {
    MdxSchema
  }
  pub fn toc() -> TocSchema {
    TocSchema
  }
  pub fn metadata() -> MetadataSchema {
    MetadataSchema
  }
  pub fn excerpt() -> ExcerptSchema {
    ExcerptSchema::default()
  }
  pub fn path() -> PathSchema {
    PathSchema::default()
  }
  pub fn slug() -> SlugSchema {
    SlugSchema::default()
  }
  pub fn unique() -> UniqueSchema {
    UniqueSchema::default()
  }
  pub fn isodate() -> IsodateSchema {
    IsodateSchema
  }
  pub fn file() -> FileSchema {
    FileSchema::default()
  }
  pub fn image() -> ImageSchema {
    ImageSchema::default()
  }
}
