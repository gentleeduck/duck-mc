...
//!
//!     cargo run -p duck-md-codegen --features bin --bin codegen                                   # default sample, both renderers
//!     cargo run -p duck-md-codegen --features bin --bin codegen -- ../samples/headings.mdx --html
//!     echo '# hi' | cargo run -p duck-md-codegen --features bin --bin codegen -- - --mdx
//!
//! Flags:
...
fn main() -> io::Result<()> {
  ...
  // Render. Codegen now emits diagnostics straight into shared engines.
  let html_engine = RefCell::new(DiagnosticEngine::new());
  let mdx_engine = RefCell::new(DiagnosticEngine::new());
  let html =
    if want_html { HtmlEmitter::render(&doc, &mut html_engine.borrow_mut()) } else { String::new() };
  let mdx =
    if want_mdx { MdxBodyEmitter::render(&doc, &mut mdx_engine.borrow_mut()) } else { String::new() };

  let lex_diags = lex_engine.borrow();
  let parse_diags = parse_engine.borrow();
  let transform_diags = transform_engine.borrow();
  let html_diags = html_engine.borrow();
  let mdx_diags = mdx_engine.borrow();
  let total = lex_diags.iter().count()
    + parse_diags.iter().count()
    + transform_diags.iter().count()
    + html_diags.iter().count()
    + mdx_diags.iter().count();
  ...
  if show_json {
    let mut out = json!({
      ...
      "errors": lex_diags.error_count() + parse_diags.error_count() + transform_diags.error_count(),
      "warnings": lex_diags.warning_count() + parse_diags.warning_count() + transform_diags.warning_count(),
    });
    if !no_ast {
  ...
  if !quiet && total > 0 {
    ...
    println!("{}", html_diags.format_all_plain(&source));
    println!("{}", mdx_diags.format_all_plain(&source));
  }
