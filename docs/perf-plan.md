# duck-md performance optimization plan
...
| Engine                      | skills.mdx   |
| --------------------------- | ------------ |
| duck-md (Rust, cargo bench) | **0.119 ms** |
| @mdx-js/mdx                 | 2.583 ms     |
| unified remark→rehype HTML  | 1.986 ms     |
| marked (md-only baseline)   | 0.136 ms     |

duck-md is already faster than `marked`, the fastest-known JS markdown parser. **21.7× faster than @mdx-js/mdx** with full MDX semantics. Little single-file headroom left at the parse layer.
...
| duck-md build | **46 ms** | 0.05 ms  | **159× faster**     |

### Sidecar bottleneck

`duck-md-core/src/engine.rs:519-552` spawns `node duck-md-sidecar/index.mjs` once **per file** when JS plugins are configured.
...
| # | File:line                              | Issue |
|---|----------------------------------------|-------|
| 1 | `duck-md-core/src/engine.rs:538`       | `Command::new("node")` per file in `run_sidecar` |
| 2 | `duck-md-core/src/engine.rs:159`       | `Pipeline::with_defaults()` rebuilt inside rayon `par_iter` per file |
| 3 | `duck-md-schema/src/primitives.rs:53`  | `fancy_regex::Regex::new(pat)` inside `parse()` — recompiled per call |
| 4 | `duck-md-codegen/src/mdx.rs` (20+ `format!`) | Each AST node allocates several fresh `String`s |
| 5 | `duck-md-codegen/src/html.rs`          | Same pattern |
| 6 | `duck-md-core/src/engine.rs:223-227`   | `serde_json::to_string_pretty` then `fs::write` (large buffered string) |
| 7 | `duck-md-schema/src/primitives.rs:36,46` | `s.chars().count()` for length checks (full O(n) walk) |
...
- **AOT schema descriptor** — already AOT-compiled in `duck-md-schema/src/compile.rs`.
...
- **Files**: `duck-md-sidecar/index.mjs`, `duck-md-core/src/engine.rs:503-552`.
...
- **Files**: `duck-md-core/src/engine.rs:158-170`.
...
- **Files**: `duck-md-schema/src/primitives.rs:5-63`.
...
- **Files**: `duck-md-codegen/src/mdx.rs` (192 lines).
...
- **Files**: `duck-md-codegen/src/html.rs` (261 lines).
...
- **Files**: `duck-md-core/src/engine.rs:223-227, 267, 291`.
...
- **Files**: `duck-md-codegen/src/escape.rs`.
...
```sh
cargo build --release -p duck-md-core --bin duck-md
...
cargo bench -p duck-md-core --bench compile 2>&1 | grep -E "time:|compile|parse"

# 2. End-to-end build at scale (no JS plugins)
cd /tmp/duck-bench-big && rm -rf .gentleduck && \
  /usr/bin/time -v <repo>/target/release/duck-md build --config duck-md.toml 2>&1 | \
...
- `duck-md-core/src/engine.rs` — U1, U2, U6
- `duck-md-sidecar/index.mjs` — U1
- `duck-md-schema/src/primitives.rs` — U3
- `duck-md-codegen/src/mdx.rs` — U4
- `duck-md-codegen/src/html.rs` — U5
- `duck-md-codegen/src/escape.rs` — U7
