...
let outcomes: Vec<(Value, Option<EngineError>)> = paths
  ...
  .map(|path| {
    ...
    if has_js_plugins(cfg) {
      compiled.html = run_sidecar(&compiled.content, cfg)?;   // ⚠ per-file Node spawn
    }
...
- `s` — the `s.*` builder bridge: each call produces a JSON descriptor fed to the Rust `compile_descriptor` (see §5).
...
- **rayon parallelism is per-file, not per-collection.** Collections run sequentially; files within a collection run on all cores. This is what gets the 159× scale lead at 999 files.
