...
- `lex_fenced_code` now requires only-whitespace after the closing
  backtick run (CommonMark §4.5). Pre-fix, `` ```tsx /Generic/ `` mid-
...
- `lex_inline_code` spans newlines per CommonMark §6.1, with bail-outs
...
- Engine `clean: true` no longer wipes `<output_dir>/.cache/` — cache
  ...
  cache. Warm full builds on `apps/duck` go 34 s → 3.3 s (~10×).
...
- **`Arc<str>` file path on every lexer span**. Was a literal `""`
  ...
  At ~50k tokens/file × 1000 files that's ~50M atomic increments.
- **Multi-line `lex_inline_code` rewrite**. The phase-5 fast path
  was a single `skip_until_any2(b'\n', b'`')` call backed by
  `memchr`; the §6.1-compliant version walks char-by-char so it can
...
velite is unchanged (still ~30-120× faster). The single-file
criterion bench (`compile fixture`) actually got *faster*: 119 µs →
111 µs (-6 %), because the dominant cost there is the parse + codegen,
...
The 10× warm number is the headline outcome of phase 6. Phase 5 had
