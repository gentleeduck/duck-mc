...
fn diagnostic_output_snapshots_are_stable() {
  ...
  assert!(link.contains("error: [P001]: inline link destination did not close before the end of the line; treating it as literal text"));
  assert!(link.contains("--> <test>:1:1"));
  assert!(link.contains("= help: add a closing `)` to finish `[text](...)`, or escape the `[` if this should stay literal"));

  let fence = parse_with_diagnostics("```\n").format_all_plain("```\n");
  assert!(fence.contains("error: [P004]: fenced code block never found a matching closing fence; treating the rest of the file as code"));
  ...
  assert!(jsx.contains("error: [P013]: JSX attribute `bar` is missing a value before the tag ended; preserving the text literally"));
