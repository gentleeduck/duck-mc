...
fn main() -> io::Result<()> {
  ...
  let html = if want_html {
    HtmlEmitter::render(&doc, &mut html_engine.borrow_mut())
  } else {
    String::new()
  };
  let mdx = if want_mdx {
    MdxBodyEmitter::render(&doc, &mut mdx_engine.borrow_mut())
  } else {
    String::new()
  };
  ...
  if !quiet && total > 0 {
    ...
    print!("{}", duck_diagnostic::format_all_smart(&html_diags, color));
    print!("{}", duck_diagnostic::format_all_smart(&mdx_diags, color));
  }
