...
fn lt_then_digit_is_text() {
    ...
    assert!(!kinds.iter().any(|k| matches!(k, TokenKind::JsxOpenTagStart)),
        "should not start JSX, got {:?}", kinds);
}
...
fn lt_then_gt_fragment_dispatches_to_jsx() {
    ...
    let kinds = lex_kinds("<></>");
    assert!(kinds.first() == Some(&TokenKind::JsxOpenTagStart),
        "expected JsxOpenTagStart, got {:?}", kinds);
}
