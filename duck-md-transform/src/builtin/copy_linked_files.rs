...
impl Transformer for CopyLinkedFiles {
  fn name(&self) -> &str { "copy-linked-files" }
...
struct Apply<'a> { config: &'a CopyLinkedFiles }
...
impl CopyLinkedFiles {
  fn publish(&self, raw: &str) -> Option<String> {
    if raw.starts_with("http://") || raw.starts_with("https://") || raw.starts_with("//") || raw.starts_with('/') {
      return None;
    }
    if raw.starts_with('#') { return None; }
    {
      let map = self.map.lock().unwrap();
      if let Some(u) = map.get(raw) { return Some(u.clone()); }
    }
    ...
    let filename = self.name_template
      .replace("[name]", stem)
