...
//!
//!     cargo run -p duck-md-transform --bin transform                          # default: ../samples/index.mdx
//!     cargo run -p duck-md-transform --bin transform -- ../samples/headings.mdx
//!     echo '# hi' | cargo run -p duck-md-transform --bin transform -- -       # stdin (use `-`)
//!
//! Flags:
...
fn main() -> io::Result<()> {
  ...
  let (label, source, input_dir, meta) = match args.first().map(String::as_str) {
    None => {
...
fn build_pipeline_from_names(csv: &str) -> Pipeline {
  ...
  for raw in csv.split(',') {
    ...
    p = match name {
      ...
      "mermaid" => p.add(duck_md_transform::Mermaid::default()),
      "npm-command" => p.add(duck_md_transform::NpmCommand),
      other => {
