...
fn build_schema_ctx(
  ...
  let mut ctx =
    duck_md_schema::Ctx::new(path.to_path_buf(), root.to_path_buf(), compiled.content.clone());
...
fn run_sidecar(markdown: &str, cfg: &EngineConfig) -> Option<String> {
  ...
  let entry =
    std::env::var("DUCK_MD_SIDECAR").ok().or_else(|| Some("duck-md-sidecar/index.mjs".into()))?;
