...
fn init_writes_config() {
  ...
  let cfg = dir.path().join("duck-md.toml");
  Command::cargo_bin("duck-md").unwrap().args(["init", "--path"]).arg(&cfg).assert().success();
...
fn init_refuses_overwrite() {
  ...
  Command::cargo_bin("duck-md").unwrap().args(["init", "--path"]).arg(&cfg).assert().failure();
}
...
fn build_runs_engine() {
  ...
  let cfg_path = dir.path().join("duck-md.toml");
  std::fs::write(&cfg_path, cfg).unwrap();
  Command::cargo_bin("duck-md")
    .unwrap()
    .args(["build", "--config"])
    .arg(&cfg_path)
    .assert()
...
fn compile_prints_json_for_a_file() {
  ...
  let assert = Command::cargo_bin("duck-md").unwrap().args(["compile"]).arg(&f).assert().success();
